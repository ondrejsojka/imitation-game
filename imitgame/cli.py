...
def cmd_test_provider(args):
    ...
    messages = [
        ...
        Message(role="user", content="Hi everyone! What do you think about AI?", actor_id="Actor 1"),
    ]
...
def main():
    ...
    play_parser.add_argument("--turns", type=int, default=3, help="Number of conversation turns")
    play_parser.add_argument("--preset", choices=list(PRESETS.keys()), help="Model preset")
    play_parser.add_argument("--models", nargs="+", help="Specific models to use")
    play_parser.add_argument("--with-prefill", action="store_true", help="Include Gemini prefill mode")
    ...
    demo_parser.add_argument("--with-prefill", action="store_true", help="Include Gemini prefill")
    demo_parser.set_defaults(func=cmd_demo)
    
    # Test provider command
    test_parser = subparsers.add_parser("test", help="Test a provider")
    test_parser.add_argument("provider", help="Provider spec (e.g., 'openai/gpt-4o-mini')")
